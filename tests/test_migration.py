...
def test_saved_bytes_and_complete_schemas_agree(current_oracle):
...
@pytest.mark.parametrize('actual,expected,kind', [
    ...
    ((0,), (False,), 'changed'), (({'n': 0},), ({'n': False},), 'changed'),
])
...
def test_migration_gate_goes_red_and_refuses_timing(current_oracle, monkeypatch, defect):
    original = vm.oracle
    class Broken:
...
def test_untested_schema_is_not_counted_as_migratable(current_oracle):
    pages = registry()
...
def test_manifest_requires_original_bytes(tmp_path):
    doc = json.loads((FIXTURES / 'manifest.json').read_text(encoding='utf-8'))
...
def test_a_transform_cannot_hide_a_lost_raw_value(current_oracle, monkeypatch):
    original = vm.oracle
    class Broken:
        def __init__(self, item): self.item = item
...
def test_reference_preserves_css_escapes_and_routes_padded_xpath():
    html = b'<p id="a ">value</p>'
    page = Page().field('id', '#a\\ ::attr(id)').many('rows', ' //p ', {'text': ' text() '})
    assert vm.oracle(page, html, 'utf-8').to_dict() == page.extract(html).to_dict() == {
        'id': 'a ', 'rows': [{'text': 'value'}],
    }
    node = vm.Selector(body=html)
    assert vm.values(node, '#a\\ ') == node.css('#a\\ ').getall()


@pytest.mark.parametrize('kind', ['first', 'all', 'join'])
def test_item_shaping_regressions_cannot_change_the_reference(current_oracle, monkeypatch, kind):
    import frostwork.page as implementation
    original = implementation._shape

    def broken(column, card, transforms=()):
        if card[0] == kind:
            return 'shaping regression'
        return original(column, card, transforms)

    monkeypatch.setattr(implementation, '_shape', broken)
    monkeypatch.setattr(vm, 'timed_pair', lambda *args: pytest.fail('timed a failing schema'))
    report = vm.verify(registry(), FIXTURES / 'manifest.json', benchmark=True)
    assert not report['ok']
    assert any(row.get('differences') for row in report['responses'])
    assert 'benchmark_skipped' in report
