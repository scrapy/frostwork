...
/// `</body>`/`</html>` are the only unconditional document closers; whether a table-family end tag may
/// unwind is decided by [`blocks_end_tag`] (row/cell machinery is transparent, a nested table is not).
...
///
/// libxml2 has two distinct end-tag scope rules:
/// - ordinary end tags cannot unwind through its table-scoped set or through an open `<div>`;
/// - table-family end tags (`</tr>`, `</tbody>`, …) may unwind their own row/section machinery, but a
///   nested `<table>` is still a hard boundary.
///
/// The `<div>` cell is name-specific (other `BLOCK` tags do not behave this way), hence the raw name
/// argument instead of folding it into the coarser `tag` id.
pub fn blocks_end_tag(open_name: &[u8], open_tid: u8, closing: &[u8]) -> bool {
    let (closing_tid, _, _) = classify(closing);
    if is_table_scoped(closing_tid) {
        open_tid == tag::TABLE
    } else {
        is_table_scoped(open_tid) || open_name.eq_ignore_ascii_case(b"div")
    }
}
...
// ---- BEGIN GENERATED by tools/gen_tree_rules.py — do not edit by hand ----
